...
We built this application with internationalization in mind. Indeed, the user locale translates 
every text (user interface, emails, validation errors, etc.).

This documentation will explain the different components that help us with that task and how they interact.
...
The [CreateEmail](../src/api/src/UseCase/CreateEmail.php) use case has a method `create`, which takes, 
...
The `CreateEmail` use case provides both `domain` and `locale` values.

### Spreadsheets

You might want to translate the headers and cell values of your XLSX exports. 
See [CreateSimpleXlsx](../src/api/src/UseCase/CreateSimpleXlsx.php) and [CreateUsersXlsx](../src/api/src/UseCase/User/CreateUsersXlsx.php)
...
the [nuxt.config.js](../src/webapp/nuxt.config.js) file.
...
Folder [locales](../src/webapp/locales) contains one JavaScript file per locale.
...
The first time the user lands on your application's root pages (`/x`), it automatically detects the browser language and 
sets the cookie `i18n_redirected` with the corresponding locale. 

If your application does not support the browser language, it uses the default locale instead.

Later, the application will use cookie `i18n_redirected`'s value to translate the user interface to 
the correct locale automatically (and redirect the user to the right path - see below).
...
If you don't know the name of your route, take a look at the file `.nuxt/router.js`. For instance:
...
In the file [Header.vue](../src/webapp/components/layouts/Header.vue), you have an example of how to update the locale.
...
Our custom plugin [i18n.js](../src/webapp/plugins/i18n.js) hooks itself on these events for:
...
The [LocaleSubscriber](../src/api/src/Infrastructure/EventSubscriber/LocaleSubscriber.php) from the API reads the 
value of this HTTP header to set the locale on its side.
...
* [Users list](../src/webapp/pages/admin/users/index.vue)
* [UsersXlsxController](../src/api/src/Infrastructure/Controller/User/UsersXlsxController.php)
...
* [i18n.js](../src/webapp/plugins/i18n.js) plugin
* [UpdateLocale.php](../src/api/src/UseCase/User/UpdateLocale.php)

# I don't need i18n

No problem! Remove the web application's locale selection ([Header.vue](../src/webapp/components/layouts/Header.vue))
and set the `DEFAULT_LOCALE` environment variable with your unique locale (see [default locale](#default-locale) chapter).

---

[Back to top](#internationalization-i18n) - [Home](/)